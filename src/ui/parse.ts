...
export const processTextInstructions = (instructionList: string) => {
  ...
  for (let line = 1; line < instructionArray.length; line += 2) {
    ...
    const orientation =
      ORIENTATIONS[ORIENTATIONS.indexOf(vehicleDataLine.split(" ")[2])];
    ...
    if (
      line + 1 < instructionArray.length &&
      instructionArray[line + 1] !== ""
    ) {
      const newVehicleData = processMovementString(
        vehicle,
        grid,
        instructionArray[line + 1]
      );
      output += `${line == 1 ? "" : "\n"}${newVehicleData.position.xPos} ${
        newVehicleData.position.yPos
      } ${newVehicleData.orientation}`;
    } else {
      output += `${line == 1 ? "" : "\n"}${vehicle.position.xPos} ${
        vehicle.position.yPos
      } ${vehicle.orientation}`;
    }
  }
